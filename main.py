...
def inline(c):
    if c.data=='reg':
    	...
    	link = types.InlineKeyboardButton(text="Перейти", url='https://t.me/mamaprovoda')
...
def photo(message):
	...
	if message.text == 'ГОТОВО':
		...
		src = '/Users/anna/desktop/tg/img/' + message.photo[1].file_id
		...
		msg = bot.send_message(message.chat.id, 'Фото успешно загружено!')
		bot.register_next_step_handler(msg, photo)
